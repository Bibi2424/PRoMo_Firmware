...
// Use a 8-bit number for the address, LSB must be 0 so for
// ORing over the last bit correctly based on reads and writes
#define VL53L0x_I2C_ADDRESS_DEFAULT 0b01010010


typedef uint8_t (*vl53l0x_i2c_read_t)(uint8_t address, uint8_t mem, uint8_t *buffer, uint8_t size);
typedef uint8_t (*vl53l0x_i2c_write_t)(uint8_t address, uint8_t mem, uint8_t const *buffer, uint8_t size);

typedef struct {
    uint32_t (*millis)(void);

    vl53l0x_i2c_read_t i2c_read;
    vl53l0x_i2c_write_t i2c_write;
...
// change current address without writing
